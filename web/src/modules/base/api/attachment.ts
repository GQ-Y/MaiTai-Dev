...
export function uploadChunk(formData: FormData) {
  return useHttp().post('/admin/attachment/upload-chunk', formData)
}

export function mergeChunk(data: { file_md5: string, chunk_total: number, filename: string }) {
  return useHttp().post('/admin/attachment/merge-chunk', data)
}
